...
export const mxUtils = {
   ...
   */
  removeCursors: function(element: Node) {
  ...
  getCurrentStyle: (function() {
    if (
      ...
      return function(element: HTMLElement) {
        // 注：只有IE和Opera支持使用CurrentStyle获取的元素计算后的样式。
        // refer: https://blog.csdn.net/weixin_42210229/article/details/81101592
        // @ts-ignore
   ...
   * 这些属性一般出现在 border、padding 等 css 样式中
   */
  parseCssNumber: function(value: string | number) {
    if (value == 'thin') {
      value = '2';
  ...
  setPrefixedStyle: (function() {
    ...
    return function(style: CSSStyleDeclaration, name: any, value: any) {
      style[name] = value;
   ...
   */
  hasScrollbars: function(node: HTMLElement) {
   ...
   */
  bind: function(scope: ThisType<any>, funct: FunctionConstructor) {
    return function() {
   ...
   */
  findNode: function(node: HTMLElement, attr: string, value: string) {
    ...
    while (node != null) {
      var result = mxUtils.findNode(node, attr, value) as Node | null;
   ...
   */
  getFunctionName: function(f: FunctionConstructor) {
    var str = null;
   ...
   */
  indexOf: function(array: Array<any>, obj: any) {
   ...
   */
  forEach: function(array: Array<any>, fn: (v: any) => void) {
   ...
   * array - Array to check for the given obj.
   */
  remove: function(obj: any, array: any[]) {
    var result = null;
   ...
   */
  isAncestorNode: function(ancestor: Node, child: Node | null) {
    var parent = child;
   ...
   */
  getChildNodes: function(node: Node, nodeType: number) {
   ...
   */
  importNode: function(doc: Document, node: HTMLElement, allChildren: boolean) {
    if (
      mxClient.IS_IE &&
  ...
  parseXml: (function() {
    if (window.DOMParser) {
      ...
      return function(xml: string) {
        var result = mxUtils.createXmlDocument() as Document;
   ...
   */
  getPrettyXml: <(node: HTMLElement, tab: string, indent: string) => string>(
    function(node, tab, indent) {
      var result = [];
      ...
      if (node != null) {
        ...
        if (node.nodeType == mxConstants.NODETYPE_TEXT) {
          ...
          if (tmp != null) {
            ...
            while (tmp != null) {
              result.push(
                mxUtils.getPrettyXml(tmp as HTMLElement, tab, indent + tab)
              );
              tmp = tmp.nextSibling;
            }
   ...
   */
  removeWhitespace: function(node: Node, before: boolean) {
    ...
    while (tmp != null && tmp.nodeType == mxConstants.NODETYPE_TEXT) {
      ...
      if (mxUtils.trim(text).length == 0 && tmp.parentNode) {
   ...
   */
  htmlEntities: function(s = '', newline = true) {
    s = String(s || '');
   ...
   * &#xa;
   */
  getXml: function(node: Node, linefeed = '&#xa') {
    var xml = '';
   ...
   */
  extractTextWithWhitespace: function(elems: HTMLElement[]) {
    ...
    function doExtract(elts: HTMLElement[]) {
   ...
   */
  replaceTrailingNewlines: function(str: string, pattern: string) {
   ...
   */
  setTextContent: function(node: HTMLElement, text: string) {
   ...
   */
  write: function(parent: HTMLElement, text: string) {
   ...
   * text - String representing the text to be added.
   */
  writeln: function(parent: HTMLElement, text: string) {
    var doc = parent && parent.ownerDocument;
   ...
   */
  br: function(parent: HTMLElement, count: number) {
    count = count || 1;
    var br = null;
   ...
   * current document.
   */
  button: function(label: string, funct: EventHandlerNonNull, doc = document) {
    var button = doc.createElement('button');
    mxUtils.write(button, label);

    mxEvent.addListener(button, 'click', function(evt: Event) {
      funct(evt);
    });
   ...
   */
  para: function(parent: HTMLElement, text: string) {
   ...
   */
  addTransparentBackgroundFilter: function(node: HTMLElement) {
    node.style.filter +=
   ...
   * key is null.
   */
  getValue: function(array: any[], key: number, defaultValue: any) {
   ...
   */
  getNumber: function(array: any, key: number, defaultValue = 0) {
    var value = array != null ? array[key] : null;
   ...
   */
  getColor: function(array: any[], key: number, defaultValue: any) {
    var value = array != null ? array[key] : null;
   ...
   */
  extend: function(ctor: Function, superCtor: Function) {
    var f = function() {};
   ...
   */
  arcToCurves: function(x0: number, y0: number, r1: number, r2: number, angle: number, largeArcFlag: number, sweepFlag: number, x: number, y: number) {
    x -= x0;
    y -= y0;
    ...
    var seg = Math.ceil(sse < 0 ? -1 * sse : sse); // 计算拆分成多少个 90° 的弧线
    var segr = dr / seg;
    var t =
   ...
   */
  ltrim: function(str: string | null, chars = '\\s') {
    chars = chars || '\\s';
   ...
   * - "\x0B" (ASCII 11 (0x0B)), a vertical tab
   */
  rtrim: function(str: string | null, chars = '\\s') {
    chars = chars || '\\s';
   ...
   * - "\x0B" (ASCII 11 (0x0B)), a vertical tab
   */
  trim: function(str: string | null, chars = '\\s') {
