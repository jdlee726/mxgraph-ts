...
export default class mxWindow extends mxEventSource {

    static activeWindow: mxWindow;
    ...
    minimize: HTMLImageElement;
    maximize: HTMLImageElement;
    closeImg: HTMLImageElement;
    image: HTMLImageElement;

    constructor(title: string, content: HTMLElement | null, x: number, y: number, width: number, height: number, minimizable = true, movable = true, replaceNode?: HTMLElement, style?: string) {
        super();
     ...
     */
    init = (x: number, y: number, width: number, height: number, style: string = 'mxWindow') => {
    ...
    setScrollable = (scrollable: boolean) => {
        ...
        if (this.contentWrapper && navigator.userAgent.indexOf('Presto/2.5') < 0) {
            if (scrollable) {
        ...
        if (resizable) {
            if (this.resize == null) {
                ...
                var start = (evt: MouseEvent) => {
                    ...
                    mxEvent.addGestureListeners(document as any, null, dragHandler, dropHandler);
                ...
                var dropHandler = (evt: MouseEvent) => {
                    if (startX != null && startY != null) {
                        startX = null;
                        startY = null;
                        mxEvent.removeGestureListeners(document as any, null, dragHandler, dropHandler);
    ...
    installMinimizeHandler = () => {
        ...
        var minimized = false;
        var maxDisplay: string | null = null;
        var height: string | null = null;
    ...
    installMaximizeHandler = () => {
        ...
        var height: string | null = null;
        var width: string | null = null;
        var minDisplay: string | null = null;
