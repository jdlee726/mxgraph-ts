...
import { mxEventHandler } from "./mxEventSource";
...
export default class mxDragSource {
     ...
     */
    dropHandler: EventHandlerOrNull = null;
     ...
     */
    previewElement: TElementNode | null = null;
     ...
     */
    currentGraph: mxGraph | null = null;
     ...
     */
    currentPoint: mxPoint | null = null;
     ...
     */
    currentGuide: mxGuide | null = null;
    ...
    constructor(element: IMxEventElement, dropHandler: EventHandlerOrNull) {
        ...
        mxEvent.addGestureListeners(element, (evt: MouseEvent) => {
            this.mouseDown(evt);
        });

        // Prevents native drag and drop
        mxEvent.addListener(element, 'dragstart', (evt: MouseEvent) => {
            mxEvent.consume(evt);
        });
     ...
     */
    getDropTarget(graph: mxGraph, x: number, y: number, evt: MouseEvent) {
     ...
     * graph.
     */
    createPreviewElement(graph: mxGraph) {
        return null;
    };
    ...
    mouseDown(evt: MouseEvent) {
        if (this.enabled && !mxEvent.isConsumed(evt) && this.mouseMoveHandler == null) {
            ...
            mxEvent.addGestureListeners(document as any, null, this.mouseMoveHandler, this.mouseUpHandler);

            if (mxClient.IS_TOUCH && !mxEvent.isMouseEvent(evt)) {
                this.eventSource = mxEvent.getSource(evt) as IMxEventElement;
    ...
    startDrag(evt: MouseEvent) {
        this.dragElement = this.createDragElement(evt) as TElementNode;
    ...
    getElementForEvent(evt: MouseEvent) {
        return ((mxEvent.isTouchEvent(evt) || mxEvent.isPenEvent(evt)) ?
            ...
            mxEvent.getSource(evt)) as TElementNode;
    };
     ...
     */
    graphContainsEvent(graph: mxGraph, evt: MouseEvent) {
    ...
    removeListeners() {
        ...
        mxEvent.removeGestureListeners(document as any, null, this.mouseMoveHandler, this.mouseUpHandler);
     ...
     * Actives the given graph as a drop target.
     */
    dragEnter(graph: mxGraph, evt: MouseEvent) {
     ...
     */
    dragExit(graph: mxGraph, evt?: MouseEvent) {
