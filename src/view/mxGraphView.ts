...
import { mxEvent, EventHandlerOrNull } from "../util/mxEvent";
...
export default class mxGraphView extends mxEventSource {
     ...
     */
    graphBounds: mxRectangle | null = null;
    ...
    moveHandler: EventHandlerOrNull;
    endHandler: EventHandlerOrNull;
 ...
 */
     ...
     * recursion.
     */
    clear = (cell?: mxCell, force = false, recurse = true) => {
        var model = this.graph.getModel();
     ...
     * Default is true.
     */
    getBoundingBox = (state: mxCellState, recurse = true) => {
        var bbox = null;
        ...
        if (state != null) {
            ...
            if (recurse) {
                ...
                for (var i = 0; i < childCount; i++) {
                    var bounds = this.getBoundingBox(this.getState(model.getChildAt(state.cell, i))) as typeof bbox;
     ...
     */
    createBackgroundPageShape = (bounds: mxRectangle) => {
        ...
        if (this.graph.pageVisible) {
            ...
            if (this.backgroundPageShape == null) {
                ...
                if (this.graph.nativeDblClickEnabled) {
                    mxEvent.addListener(this.backgroundPageShape.node, 'dblclick', (evt: MouseEvent) => {
     ...
     * 
     * (code)
     * redrawBackground = function(backgroundImage, bg)
     * {
     ...
     */
    validateCellState = (cell: mxCell, recurse = true) => {
        var state = null;
     ...
     * constraint - <mxConnectionConstraint> that specifies the connection.
     */
    getFixedTerminalPoint = (edge: mxCellState, terminal: mxCellState, source: boolean, constraint: mxConnectionConstraint) => {
        var pt = null;
     ...
     */
    transformControlPoint = (state: mxCellState, pt: mxPoint) => {
     ...
     * associated.
     */
    isLoopStyleEnabled = (edge: mxCellState, points: mxPoint[], source: mxCellState, target: mxCellState) => {
     ...
     */
    getEdgeStyle = (edge: mxCellState, points: mxPoint[], source: mxCellState, target: mxCellState) => {
     ...
     * source - Boolean indicating if start is the source terminal state.
     */
    getFloatingTerminalPoint = (edge: mxCellState, start: mxCellState, end: mxCellState, source: boolean) => {
     ...
     */
    getTerminalPort = (state: mxCellState, terminal: mxCellState, source: boolean) => {
     ...
     */
    getPerimeterPoint = (terminal: mxCellState, next: mxPoint, orthogonal: boolean, border?: boolean) => {
        var point = null;
     ...
     * getPerimeterBounds = function(terminal, edge, isSource)
     * {
     *   var model = this.graph.getModel();
    ...
    installListeners = () => {
        ...
        if (container != null) {
            ...
            mxEvent.addGestureListeners(container, (evt: MouseEvent) => {
    ...
    updateHtmlCanvasSize = (width: number, height: number) => {
        if (this.canvas && this.graph.container != null) {
     ...
     */
    createVmlPane = (width: string, height: string) => {
