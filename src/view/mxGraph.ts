...
import mxEventSource, { TSender, mxEventHandler } from "../util/mxEventSource";
import mxEventObject from "../util/mxEventObject";
import mxMouseEvent from "../util/mxMouseEvent";
import { mxEvent } from "../util/mxEvent";
import { mxConstants, IPlainObject } from "../util/mxConstants";
...
export default class mxGraph extends mxEventSource {
     ...
     */
    cellRenderer: mxCellRenderer | null = null;
     ...
     */
    multiplicities: mxMultiplicities | null = null;
    ...
    graphModelChangeListener: mxEventHandler;
    ...
    constructor(container: HTMLElement, model?: mxGraphModel, renderHint: string | null = null, stylesheet: mxStylesheet | null = null) {
        super();
        ...
        this.graphModelChangeListener = (sender: TSender, evt: mxEventObject) => {
    ...
    init = (container: HTMLElement) => {
        ...
        if (mxClient.IS_IE) {
            mxEvent.addListener(window as any, 'unload', () => {
                this.destroy();
            });
     ...
     */
    setSelectionModel = (selectionModel: mxSelectionModel) => {
     ...
     */
    getSelectionCellsForChanges = (changes: TChange[] ) => {
     ...
     */
    graphModelChanged = (changes: TChange[]) => {
        for (var i = 0; i < changes.length; i++) {
     ...
     */
    processChange = function (change: TChange) {
     ...
     */
    removeCellOverlay = (cell: mxCell, overlay: mxCellOverlay) => {
        if (overlay == null) {
     ...
     */
    setCellWarning = (cell: mxCell, warning: string, img?: mxImage, isSelect: boolean = false) => {
        if (warning != null && warning.length > 0) {
            ...
            if (isSelect) {
                overlay.addListener(mxEvent.CLICK,
                    (sender: TSender, evt: mxEventObject) => {
                        if (this.isEnabled()) {
     ...
     * evt - Optional mouse event that triggered the editing.
     */
    startEditingAtCell = (cell: mxCell | null, evt?: MouseEvent | null) => {
     ...
     */
    cellLabelChanged = (cell: mxCell, value: any, autoSize: boolean) => {
        this.model.beginUpdate();
        try {
        ...
        if (this.isEnabled() && !mxEvent.isConsumed(evt) && !mxe.isConsumed()) {
            if (cell != null) {
                if (this.isTransparentClickEvent(evt)) {
                    ...
                    var tmp = this.getCellAt(me.graphX, me.graphY, null, null, null, (state: mxCellState) => {
     ...
     */
    getPreferredPageSize = (bounds: mxRectangle, width: number, height: number) => {
     ...
     */
    fit = (border = this.getBorder(), keepOrigin = false, margin = 0, enabled = true, ignoreWidth = false, ignoreHeight = false, maxHeight: number | null = null) => {
    ...
    updatePageBreaks = (visible: boolean, width: number, height: number) =>  {
        ...
        var drawPageBreaks = (breaks?: mxPolyline[]) => {
            if (breaks) {
     ...
     */
    postProcessCellStyle =  (style?: IPlainObject) => {
        if (style != null) {
     ...
     * selection cells.
     */
    setCellStyle = (style: IPlainObject, cells?: mxCell[]) => {
     ...
     * the selection cell.
     */
    toggleCellStyle = (key: string, defaultValue?: boolean, cell?: mxCell) => {
     ...
     * the selection cells.
     */
    setCellStyles = (key: string, value: any, cells: mxCell[]) => {
        cells = cells || this.getSelectionCells();
     ...
     * the selection cells.
     */
    setCellStyleFlags = (key: string, flag: number, value: any, cells: mxCell[]) => {
        cells = cells || this.getSelectionCells();
     ...
     */
    alignCells = (align: string, cells: mxCell[], param?: number) => {
        if (cells == null) {
     ...
     */
    groupCells = (group: mxCell, border: number, cells: mxCell[]) => {
        if (cells == null) {
            cells = mxUtils.sortCells(this.getSelectionCells(), true);
        }
     ...
     */
    getBoundsForGroup = (group: mxCell, children: mxCell[], border: number) => {
     ...
     */
    updateGroupBounds = (cells: mxCell[] | null, border = 0, moveGroup = false, topBorder = 0, rightBorder = 0, bottomBorder = 0, leftBorder = 0) => {
        if (cells == null) {
            cells = this.getSelectionCells();
        }
     ...
     */
    cloneCell = (cell: mxCell, allowInvalidEdges = true, mapping?: Object, keepPosition = false) => {
    ...
    insertVertex = (parent: mxCell, id: string, value: any,
        x: number, y: number, width: number, height: number, style: string, relative = false) => {
     ...
     * style - Optional string that defines the cell style.
     */
    insertEdge = (parent: mxCell, id?: string, value?: any, source?: mxCell, target?: mxCell, style?: string) => {
     ...
     * 
     */
    createEdge = (parent: mxCell, id?: string, value?: any, source?: mxCell, target?: mxCell, style?: string) => {
        // Creates the edge
     ...
     */
    addEdge = (edge: mxCell, parent?: mxCell, source?: mxCell, target?: mxCell, index?: number) => {
     ...
     */
    addCells = (cells: mxCell[], parent?: mxCell, index?: number, source?: mxCell, target?: mxCell, absolute = false) => {
        if (parent == null) {
     ...
     */
    cellsAdded = (cells: mxCell[], parent?: mxCell, index?: number, source?: mxCell | null, target?: mxCell | null, absolute?: boolean | null, constrain?: boolean | null, extend?: boolean | null) => {
     ...
     */
    autoSizeCell = (cell: mxCell, recurse = true) => {
        ...
        if (cells != null && cells.length > 0) {
            ...
            try {
                ...
                for (var i = 0; i < cells.length; i++) {
                    ...
                    var disconnectTerminal = (edge: mxCell, source: boolean) => {
     ...
     */
    splitEdge = (edge: mxCell, cells: mxCell[], newEdge: mxCell, dx: number = 0, dy: number = 0) => {
     ...
     */
    toggleCells = (show: boolean, cells?: mxCell[], includeEdges = true) => {
        if (cells == null) {
            cells = this.getSelectionCells();
        }
     ...
     */
    foldCells = (collapse: boolean, recurse = false, cells?: mxCell[], checkFoldable = false, evt?: MouseEvent) => {
     ...
     * checked. Default is false.
     */
    cellsFolded = (cells: mxCell[], collapse: boolean, recurse = false, checkFoldable = false) => {
        if (cells != null && cells.length > 0) {
            this.model.beginUpdate();
            try {
                for (var i = 0; i < cells.length; i++) {
     ...
     */
    updateCellSize = (cell: mxCell, ignoreChildren = false) => {
     ...
     */
    resizeCell = (cell: mxCell, bounds: mxRectangle, recurse?: boolean) => {
     ...
     */
    resizeCells = (cells: mxCell[], bounds: mxRectangle[], recurse = this.isRecursiveResize()) => {
     ...
     * mapping - Optional mapping for existing clones.
     */
    moveCells = (cells: mxCell[], dx = 0, dy = 0, clone = false, target?: mxCell, evt?: MouseEvent, mapping?: Object) => {
        dx = (dx != null) ? dx : 0;
        dy = (dy != null) ? dy : 0;
     ...
     */
    constrainChild = (cell: mxCell, sizeFirst = true) => {
     ...
     */
    getOutlineConstraint = (point: mxPoint, terminalState: mxCellState, me: mxMouseEvent) => {
     ...
     */
    getAllConnectionConstraints = (terminal: mxCellState, source?: boolean) => {
     ...
     */
    getConnectionConstraint = (edge: mxCellState, terminal: mxCellState, source: boolean) => {
        var point = null;
     ...
     */
    getConnectionPoint = (vertex: mxCellState, constraint: mxConnectionConstraint, round = true) => {
        var point = null;
     ...
     * connection.
     */
    connectCell = (edge: mxCell, terminal: mxCell, source: boolean, constraint?: mxConnectionConstraint) => {
        this.model.beginUpdate();
        try {
     ...
     */
    getTerminalForPort = (cell: mxCell, source?: boolean) => {
     ...
     */
    getCellBounds = (cell: mxCell, includeEdges = false, includeDescendants = false) => {
        var cells = [cell];
     ...
     */
    getBoundingBoxFromGeometry = function (cells: mxCell[], includeEdges = false) {
     ...
     */
    snapDelta = function (delta, bounds, ignoreGrid, ignoreHorizontal, ignoreVertical) {
        var t = this.view.translate;
        var s = this.view.scale;
     ...
     */
    center = function (horizontal, vertical, cx, cy) {
     ...
     */
    scrollCellToVisible = function (cell, center) {
     ...
     */
    scrollRectToVisible = function (rect) {
        var isChanged = false;
     ...
     */
    isCellConnectable = function (cell) {
     ...
     */
    isTransparentClickEvent = function (evt) {
        return false;
    };
     ...
     * Returns true if the given mouse event should be aligned to the grid.
     */
    isConstrainedEvent = function (evt) {
     ...
     */
    isIgnoreTerminalEvent = function (evt) {
        return false;
    };
     ...
     */
    isEdgeValid = function (edge, source, target) {
     ...
     * target - <mxCell> that represents the target terminal.
     */
    getEdgeValidationError = function (edge, source, target) {
     ...
     * target - <mxCell> that represents the target terminal.
     */
    validateEdge = function (edge, source, target) {
        return null;
    };
     ...
     */
    validateGraph = function (cell, context) {
        cell = (cell != null) ? cell : this.model.getRoot();
     ...
     */
    getCellValidationError = function (cell) {
     ...
     * context - Object that represents the global validation state.
     */
    validateCell = function (cell, context) {
        return null;
    };
     ...
     */
    setBackgroundImage = (image: mxImage | null) => {
     ...
     */
    getFoldingImage = function (state) {
     ...
     */
    getTooltip = function (state, node, x, y) {
        var tip = null;
     ...
     */
    getCursorForMouseEvent = function (me) {
     ...
     */
    getCursorForCell = function (cell) {
        return null;
    };
     ...
     */
    getStartSize = function (swimlane) {
     ...
     */
    isTransparentState = function (state) {
        var result = false;
     ...
     */
    setResizeContainer = function (value) {
     ...
     */
    setEscapeEnabled = function (value) {
     ...
     */
    setInvokesStopCellEditing = function (value) {
     ...
     */
    setEnterStopsCellEditing = function (value) {
     ...
     */
    getCloneableCells = function (cells) {
     ...
     */
    setCellsCloneable = function (value) {
     ...
     * Returns the cells which may be exported in the given array of cells.
     */
    getExportableCells = function (cells) {
        return this.model.filterCells(cells, mxUtils.bind(this, function (cell) {
     ...
     */
    getImportableCells = function (cells) {
        return this.model.filterCells(cells, mxUtils.bind(this, function (cell) {
     ...
     */
    setCellsSelectable = function (value) {
     ...
     * Returns the cells which may be exported in the given array of cells.
     */
    getDeletableCells = function (cells) {
        return this.model.filterCells(cells, mxUtils.bind(this, function (cell) {
     ...
     */
    setCellsDeletable = function (value) {
     ...
     */
    getMovableCells = function (cells) {
        return this.model.filterCells(cells, mxUtils.bind(this, function (cell) {
     ...
     */
    setCellsMovable = function (value) {
     ...
     */
    setVertexLabelsMovable = function (value) {
     ...
     */
    setEdgeLabelsMovable = function (value) {
     ...
     */
    setSwimlaneNesting = function (value) {
     ...
     */
    setSwimlaneSelectionEnabled = function (value) {
     ...
     */
    setAllowDanglingEdges = function (value) {
     ...
     */
    setConnectableEdges = function (value) {
     ...
     */
    setCloneInvalidEdges = function (value) {
     ...
     * when moved.
     */
    setDisconnectOnMove = function (value) {
     ...
     * of cells into other cells.
     */
    setSplitEnabled = function (value) {
     ...
     * cells.
     */
    setCellsResizable = function (value) {
     ...
     */
    isTerminalPointMovable = function (cell, source) {
        return true;
    };
     ...
     * edges.
     */
    setCellsBendable = function (value) {
     ...
     * editing.
     */
    setCellsEditable = function (value) {
     ...
     * disconnected.
     */
    isCellDisconnectable = function (cell, terminal, source) {
     ...
     */
    setCellsDisconnectable = function (value) {
     ...
     */
    isValidConnection = function (source, target) {
     ...
     * automatically.
     */
    setAutoSizeCells = function (value) {
     ...
     */
    setExtendParents = function (value) {
     ...
     */
    isExtendParentsOnAdd = function (cell) {
     ...
     */
    setExtendParentsOnAdd = function (value) {
     ...
     * value - New boolean value for <extendParentsOnAdd>.
     */
    setExtendParentsOnMove = function (value) {
     ...
     */
    isConstrainChild = function (cell) {
     ...
     */
    setConstrainChildren = function (value) {
     ...
     */
    setConstrainRelativeChildren = function (value) {
     ...
     */
    setAllowNegativeCoordinates = function (value) {
     ...
     */
    isAllowOverlapParent = function (cell) {
        return false;
    };
     ...
     * Returns the cells which are movable in the given array of cells.
     */
    getFoldableCells = function (cells, collapse) {
        return this.model.filterCells(cells, mxUtils.bind(this, function (cell) {
     ...
     */
    isCellFoldable = function (cell, collapse) {
        var state = this.view.getState(cell);
        var style = (state != null) ? state.style : this.getCellStyle(cell);
     ...
     * evt - Mouseevent that triggered the invocation.
     */
    isValidDropTarget = function (cell, cells, evt) {
     ...
     * evt - Mouseevent that triggered the invocation.
     */
    isSplitTarget = function (target, cells, evt) {
     ...
     */
    getDropTarget = function (cells, evt, cell, clone) {
     ...
     */
    setDefaultParent = function (cell) {
     ...
     */
    getSwimlaneAt = function (x: number, y: number, parent?: mxCell) {
     ...
     */
    hitsSwimlaneContent = function (swimlane, x, y) {
     ...
     * be returned. Default is false.
     */
    getChildCells = (parent: mxCell, vertices = false, edges = false) => {
     ...
     * returned.
     */
    getConnections = function (cell, parent) {
     ...
     * returned.
     */
    getIncomingEdges = function (cell, parent) {
     ...
     * returned.
     */
    getOutgoingEdges = function (cell, parent) {
     ...
     * Default is false
     */
    getEdges = function (cell, parent, incoming, outgoing, includeLoops, recurse) {
     ...
     */
    isValidAncestor = function (cell, parent, recurse) {
     ...
     */
    getPointForEvent = function (evt, addOffset) {
     ...
     * from the origin should be returned.
     */
    getCellsBeyond = function (x0, y0, parent, rightHalfpane, bottomHalfpane) {
        var result = [];
     ...
     */
    findTreeRoots = function (parent, isolate, invert) {
     ...
     */
    traverse = function (vertex, directed, func, edge, visited, inverse) {
     ...
     */
    setSelectionCells = function (cells) {
     ...
     */
    addSelectionCell = function (cell) {
     ...
     */
    addSelectionCells = function (cells) {
     ...
     */
    removeSelectionCell = function (cell) {
     ...
     */
    removeSelectionCells = function (cells) {
     ...
     */
    selectCell = function (isNext, isParent, isChild) {
     ...
     */
    selectAll = function (parent, descendants) {
        parent = parent || this.getDefaultParent();
     ...
     */
    selectVertices = function (parent, selectGroups) {
     ...
     * selected. Default is false.
     */
    selectCells = function (vertices, edges, parent, selectGroups) {
        parent = parent || this.getDefaultParent();
     ...
     * evt - Optional mouseevent that triggered the selection.
     */
    selectCellsForEvent = function (cells, evt) {
        if (this.isToggleEvent(evt)) {
     ...
     */
    createVertexHandler = function (state) {
     ...
     * state - <mxCellState> to create the handler for.
     */
    createEdgeHandler = function (state, edgeStyle) {
        var result = null;
     ...
     * state - <mxCellState> to create the handler for.
     */
    createEdgeSegmentHandler = function (state) {
     ...
     * state - <mxCellState> to create the handler for.
     */
    createElbowEdgeHandler = function (state) {
     ...
     */
    updateMouseEvent = function (me, evtName) {
