...
import { mxEvent, IMxEventElement } from "../util/mxEvent";
 ...
 * 
 * mxEvent.addListener(text, 'keydown', (evt: MouseEvent) =>
 * {
...
export default class mxCellEditor {
    ...
    zoomHandler: mxEventHandler | null;
    changeHandler: mxEventHandler | null;
     ...
     */
    applyValue = (state: mxCellState, value: any) => {
     ...
     */
    getInitialValue = (state: mxCellState, trigger: MouseEvent | null) => {
     ...
     */
    installListeners = (elt: IMxEventElement) => {
        // Applies value if text is dragged
        // LATER: Gesture mouse events ignored for starting move
        mxEvent.addListener(elt, 'dragstart', (evt: MouseEvent) => {
            this.graph.stopEditing(false);
            mxEvent.consume(evt);
        });

        // Applies value if focus is lost
        mxEvent.addListener(elt, 'blur', (evt: MouseEvent) => {
            if (this.blurEnabled) {
                this.focusLost(evt);
            }
        });
        ...
        mxEvent.addListener(elt, 'keydown', (evt: KeyboardEvent) => {
            if (!mxEvent.isConsumed(evt)) {
                ...
                else if ((evt as any).keyCode == 27 /* Escape */) {
        ...
        var keypressHandler = (evt: MouseEvent) => {
            if (this.editingCell != null) {
                // Clears the initial empty label on the first keystroke
                // and workaround for FF which fires keypress for delete and backspace
                if (this.clearOnChange && elt.innerHTML == this.getEmptyLabelText() &&
                    (!mxClient.IS_FF || ((evt as any).keyCode != 8 /* Backspace */ && (evt as any).keyCode != 46 /* Delete */))) {
        ...
        var keyupHandler = (evt: MouseEvent) =>{
            if (this.editingCell != null) {
        ...
        var resizeHandler = (evt: MouseEvent) => {
        ...
        mxEvent.addListener(window as any, 'resize', resizeHandler);
     ...
     * always returns null.
     */
    getBackgroundColor = (state: mxCellState) => {
        return null;
    };
     ...
     */
    startEditing = (cell: mxCell, trigger: MouseEvent | null) => {
        this.stopEditing(true);
        this.align = null;
    ...
    clearSelection = () => {
        ...
        if (window.getSelection) {
            ...
            selection = document.selection as any;
        }
    ...
    stopEditing = (cancel = false) => {
        ...
        if (this.editingCell != null && this.textarea) {
     ...
     */
    prepareTextarea = () => {
        if (this.textarea && this.textarea.lastChild != null &&
     ...
     */
    getMinimumSize = (state: mxCellState) => {
        var scale = this.graph.getView().scale;
     ...
     */
    getEditorBounds = (state: mxCellState) => {
        var isEdge = this.graph.getModel().isEdge(state.cell);
        var scale = this.graph.getView().scale;
