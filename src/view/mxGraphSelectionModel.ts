 ...
 */
export default class mxGraphSelectionModel extends mxEventSource {
     ...
     */
    getFirstSelectableCell = (cells: mxCell[]) => {
        if (cells != null) {
            for (var i = 0; i < cells.length; i++) {
                if (this.graph.isCellSelectable(cells[i])) {
                    return cells[i];
     ...
     */
    changeSelection = (added: mxCell[] | null, removed: mxCell[] | null) => {
        if ((added != null &&
            added.length > 0 &&
            added[0] != null) ||
            (removed != null &&
