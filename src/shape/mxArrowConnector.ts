 ...
 */
export default class mxArrowConnector extends mxShape {
    ...
    constructor(points: mxPoint[] | null, fill: string | null, stroke: string | null, strokewidth: number | null, arrowWidth: number | null, spacing: number | null, endSize: number | null) {
        super();
        this.points = points;
        this.fill = fill;
        this.stroke = stroke;
    ...
    paintEdgeShape(c: TCanvas2D, pts: mxPoint[]) {
        // Geometry of arrow
        var strokeWidth = this.strokewidth as number;
     ...
     * Paints the line shape.
     */
    paintMarker(c: TCanvas2D, ptX: number, ptY: number, nx: number, ny: number, size: number, arrowWidth: number, edgeWidth: number, spacing: number, initialMove: boolean) {
