...
export default class mxDoubleEllipse extends mxShape {
    ...
    paintForeground(c: TCanvas2D, x: number, y: number, w: number, h: number) {
        if (!this.outline) {
            var margin = mxUtils.getValue(this.style, mxConstants.STYLE_MARGIN, Math.min(3 + (this.strokewidth as number), Math.min(w / 5, h / 5)));
            x += margin;
            y += margin;
            w -= 2 * margin;
            h -= 2 * margin;
    ...
    getLabelBounds(rect: mxRectangle) {
        var margin = (mxUtils.getValue(this.style, mxConstants.STYLE_MARGIN, Math.min(3 + (this.strokewidth as number),
