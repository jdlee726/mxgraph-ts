 ...
 */
export default class mxText extends mxShape {
     ...
     */
    lastValue: string | HTMLElement | null;
    ...
    margin: mxPoint;
    unrotatedBoundingBox: mxRectangle | null;

    constructor(value: string, bounds: mxRectangle, align: string | null, valign: string | null, color: string | null,
        family: string | null, size: number | null, fontStyle: number | null, spacing: string | number | null, spacingTop: string | number | null, spacingRight: string | number | null,
        spacingBottom: string | number | null, spacingLeft: string | number | null, horizontal: boolean | null, background: string | null, border: string  | null,
        wrap: boolean | null, clipped: boolean | null, overflow: string | null, labelPadding: number | null, textDirection: string | null) {
            super();
        this.value = value;
        this.bounds = bounds;
    ...
    paint = (c: TCanvas2D, update?: boolean) => {
        ...
        if (update) {
            (c as mxSvgCanvas2D).updateText(x, y, w, h, this.align, this.valign, this.wrap, this.overflow,
            ...
            c.text(x, y, w, h, val as string, this.align, this.valign, this.wrap, fmt,
    ...
    redraw(){
        if (this.visible && this.checkBounds() && this.cacheEnabled && this.lastValue == this.value &&
            ...
            if (this.node!.nodeName == 'DIV' && (this.isHtmlAllowed() || !mxClient.IS_VML)) {
                ...
                if (canvas && (canvas as mxSvgCanvas2D).updateText != null) {
    ...
    getAutoDirection = () => {
        ...
        var tmp = /[A-Za-z\u05d0-\u065f\u066a-\u06ef\u06fa-\u07ff\ufb1d-\ufdff\ufe70-\ufefc]/.exec(this.value as string);
    ...
    getContentNode = () => {
        ...
        if (result != null) {
            ...
            if (result.ownerSVGElement == null) {
                result = this.node!.firstChild!.firstChild as TElementNode;
            }
            else {
                // Innermost DIV that contains the actual content
                result = result.firstChild!.firstChild!.firstChild!.firstChild!.firstChild as TElementNode;
            }
        }
        ...
        if (!this.ignoreStringSize && node != null && this.overflow != 'fill' && (!this.clipped ||
            ...
            if (node!.ownerSVGElement != null) {
                if (node!.firstChild != null && node!.firstChild.firstChild != null &&
                    ...
                    node = node!.firstChild.firstChild.firstChild!.firstChild as TElementNode;
                    oh = (node as any).offsetHeight * this.scale;

                    if (this.overflow == 'width') {
                        ow = this.boundingBox!.width;
                    }
                    else {
                        ow = (node as any).offsetWidth * this.scale;
                    }
                }
                else {
                    try {
                ...
                if (this.offsetWidth != null && this.offsetHeight != null) {
                    ...
                    var sizeDiv = node as unknown as HTMLElement;
     ...
     */
    configureCanvas(c: TCanvas2D, x: number, y: number, w: number, h: number) {
    ...
    getHtmlValue = () => {
        ...
        // Handles trailing newlines to make sure they are visible in rendering output
        val = mxUtils.replaceTrailingNewlines(val, '<div><br></div>');
        val = (this.replaceLinefeeds) ? (val as string).replace(/\n/g, '<br/>') : val;
    ...
    redrawHtmlShapeWithCss3 = () => {
        ...
        mxSvgCanvas2D.createCss(w, h, this.align, this.valign, this.wrap, this.overflow, this.clipped,
            ...
            flex, block, this.scale, (dx, dy, flex, item, block, ofl) => {
                ...
                var html = (mxUtils.isNode(this.value)) ? (this.value as HTMLElement).outerHTML : this.getHtmlValue();

                if (this.node!.firstChild == null) {
                    this.node!.innerHTML = '<div><div>' + html + '</div></div>';
                }

                (this.node!.firstChild!.firstChild as HTMLElement).setAttribute('style', block);
                (this.node!.firstChild as HTMLElement).setAttribute('style', item);
            });
    };
    ...
    updateInnerHtml(elt: HTMLElement) {
        if (mxUtils.isNode(this.value)) {
            elt.innerHTML = (this.value as HTMLElement).outerHTML;
        }
        else {
            var val = this.value;
    ...
    updateHtmlFilter = () => {
        ...
        if (td != null) {
            ...
            sizeDiv = td as TElementNode;

            if (sizeDiv.firstChild != null && sizeDiv.firstChild.nodeName == 'DIV') {
                sizeDiv = sizeDiv.firstChild as TElementNode;
        ...
        else if (sizeDiv && sizeDiv.firstChild != null && sizeDiv.firstChild.nodeName == 'DIV') {
            sizeDiv = sizeDiv.firstChild as TElementNode;
    ...
    updateValue = () => {
        if (mxUtils.isNode(this.value)) {
            this.node!.innerHTML = '';
            this.node!.appendChild(this.value as TElementNode);
        }
        else {
            var val = this.value;
            ...
            this.node!.innerHTML = val as string;
     ...
     * Updates the HTML node(s) to reflect the latest bounds and scale.
     */
    updateSize(node: TElementNode, enableWrap: boolean) {
        var w = Math.max(0, Math.round(this.bounds!.width / this.scale));
        var h = Math.max(0, Math.round(this.bounds!.height / this.scale));
        var style = node!.style;
