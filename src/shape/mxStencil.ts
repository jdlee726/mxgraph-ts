 ...
 */
export default class mxStencil extends mxShape {
    ...
    parseDescription() {
        ...
        this.fgNode = this.desc.getElementsByTagName('foreground')[0] as TElementNode;
        this.bgNode = this.desc.getElementsByTagName('background')[0] as TElementNode;
     ...
     */
    parseConstraint(node: TElementNode) {
     ...
     */
    evaluateTextAttribute(node: TElementNode, attribute: string, shape: mxShape) {
     ...
     */
    evaluateAttribute(node: TElementNode, attribute: string, shape: mxShape) {
     ...
     */
    drawShape(canvas: TCanvas2D, shape: mxShape, x: number, y: number, w: number, h: number) {
     ...
     * Draws this stencil inside the given bounds.
     */
    drawChildren(canvas: TCanvas2D, shape: mxShape, x: number, y: number, w: number, h: number, node: TElementNode, aspect: mxRectangle, disableShadow: boolean, paint: boolean) {
        if (node != null && w > 0 && h > 0) {
            var tmp = node.firstChild as TElementNode;
            ...
            while (tmp != null) {
                ...
                tmp = tmp.nextSibling as TElementNode;
            }
        }
    };
     ...
     * Draws this stencil inside the given bounds.
     */
    drawNode(canvas: TCanvas2D, shape: mxShape, node: TElementNode, aspect: mxRectangle, disableShadow: boolean, paint: boolean) {
        ...
        if (name == 'save') {
            ...
            if (name == 'path') {
                ...
                if (node.getAttribute('rounded') == '1') {
                    ...
                    var childNode = node.firstChild as TElementNode;
                    ...
                    while (childNode != null) {
                        ...
                        childNode = childNode.nextSibling as TElementNode;
                    }
